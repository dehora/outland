...
public class UriBuilderTest {
  ...
  public void buildMultiQuery() {
    ...
    assertEquals(
        "http://example.org?offset=3&foo=a&foo=b&foo=c&limit=2&owning_application=acme",
        UriBuilder.builder("http://example.org")
            .query("foo", "a", "b", "c")
            .query("limit", "2")
            .query("offset", "3")
  ...
  public void buildMultiQueryWithQueryParamsReplacing() {
    ...
    assertEquals(
        "http://example.org?offset=10&foo=a&foo=b&foo=c&limit=20&owning_application=acme",
  ...
  public void buildPathAndQuery() {
    assertEquals("http://example.org/foo/one?a=1&b=2&c=3",
        UriBuilder.builder("http://example.org")
            .path("foo")
            .path("one")
            .query("a", "1")
