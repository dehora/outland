...
public class FeatureStoreRocksDbTest {
  ...
  public void testDb() throws Exception {
    ...
    Feature feature = Feature.newBuilder().setKey("key1").setNamespace("app1").build();

    db.put(feature);

    final FeatureCollection featureCollection = db.findAll("app1");
    assertEquals("key1", featureCollection.getItems(0).getKey());
    assertEquals("app1", featureCollection.getItems(0).getNamespace());
  ...
  public void testDbMixedApps() throws Exception {
    ...
    Feature feature1 = Feature.newBuilder().setKey("key1").setNamespace("app1").build();
    Feature feature1_ = Feature.newBuilder().setKey("key10").setNamespace("app1").build();

    // this has an app id that also starts with app1 to check our scans don't match both
    Feature feature10 = Feature.newBuilder().setKey("key1").setNamespace("app10").build();
    Feature feature10_ = Feature.newBuilder().setKey("key10").setNamespace("app10").build();

    Feature feature2 = Feature.newBuilder().setKey("key2").setNamespace("app2").build();
    Feature feature2_ = Feature.newBuilder().setKey("key20").setNamespace("app2").build();

    Feature feature3 = Feature.newBuilder().setKey("key3").setNamespace("app3").build();
    Feature feature3_ = Feature.newBuilder().setKey("key30").setNamespace("app3").build();
  ...
  public void testDbReload() throws Exception {
    ...
    assertEquals("app2", featureCollection.getItems(0).getNamespace());
    ...
    assertEquals("app1", featureCollection1.getItems(0).getNamespace());
    ...
    assertEquals("app2", featureCollection2.getItems(0).getNamespace());
