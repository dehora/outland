...
class NamespaceValidator {
  ...
  void validateNamespaceRegistrationThrowing(Namespace namespace) throws ServiceException {
    if (namespace.getOwners().getItemsCount() == 0) {
      throw new ServiceException(Problem.clientProblem("no_owner_for_namespace",
          "A namespace must have at least one one owner", 422));
    }
